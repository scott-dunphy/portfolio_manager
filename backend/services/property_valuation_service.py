...
def calculate_property_valuation(property_obj: Property) -> Dict[str, object]:
    ...
    extended_end = _month_end(end_month + relativedelta(months=11))
    ...
    forward_noi = _calculate_forward_noi(months, monthly_noi)
...
def _project_monthly_noi(
    ...
    while current <= end_month:
        if (purchase_month and current < purchase_month) or (exit_month and current > exit_month):
            monthly_noi = 0.0
        elif use_manual:
            entry = manual_by_month.get((current.year, current.month))
            if entry:
                monthly_noi = entry.annual_noi or 0.0
            else:
                annual_entry = manual_by_year.get(current.year)
                monthly_noi = (
                    (annual_entry.annual_noi or 0.0) / 12.0 if annual_entry and annual_entry.annual_noi is not None else 0.0
                )
        else:
            ...
            if initial_noi is None:
                ...
                years_elapsed = months_since_start / 12.0
                annual_noi = initial_noi * ((1 + growth) ** years_elapsed)
...
def _calculate_forward_noi(months: List[date], monthly_noi: Dict[date, float]) -> Dict[date, float]:
